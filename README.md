...
<!--  #목록   -->
## 1. 요구사항
+ 보험업무시스템에서 필요한 서비스별 KEY를 관리하는 시스템 개발
+ 새로운 KEY정보 등록/ 각 KEY별 신규 KEY 발급기능이 필요함
+ KEY는 문자형 1종, 숫자행 2종(MysqlKeyGenerator,GenericKeyGenerator)으로 생성됨
</br>  
...
### 3.1 설계방향
  + 인스턴스기반으로 KEY를 생성하도록 설계(다양한 업무의 요구사항을 수요할 수 있도록 확장성 중심으로 설계)
  + KEY는 업무코드+년도+숫자형(문자형) 조합으로 생성. 
     (순수한 숫자나 문자KEY를 사용하지 않는 이유 : 실물서식이나 현장에도 구분이 용이하도록 하기위함)  
  + KEY이력정보테이블을 통하여 KEY발급 트랜드를 분석할수 있도록 함.

### 3.2 개발방향
  + 문자형 : 실시간 문자형KEY발급시 성능상 문제가 예상됨. 코드테이블을 생성하여 일괄생성후 해당테이블을 조회하여 사용
...
KEY_MST만으로 KEY발급이 가능함(단독만으로 운영시 속도에 유리). 데이터분석을 위해 발급이력이용하여 활용가능
...
### 5-2. 패키지별 기능설명
+ PC(Process Component) : 외부에 제공되는 서비스컴포넌트. 업무프로세스중심의 로직이 담김
+ EC(Entity Component)  : 업무로직을 관리하는 단위 컴포넌트
+ UC(Utility Component) : 업무로직이 없는 간단한 함수형 컴포넌트
...
+ KEY팩토리EC(KeyFactoryEC) - KEY원부테이블의 기준에 따라  키유형별 발급함수를 호출
